...
* 職種
  * SRE/バックエンドエンジニア
* やったこと
  * サービス信頼性の向上を目指し、SLO/SLA設定、エラーバジェット運用を実施
  * 高トラフィック環境におけるバックエンドAPIのパフォーマンス最適化
  * AWSやGCPを活用したインフラの自動化と運用最適化
  * サーバーレスアーキテクチャの設計と運用
  * PostgreSQLのパフォーマンスチューニングと最適化
  * Kubernetesを用いたマイクロサービスの運用と管理
  * データベースの可観測性向上、メトリクス収集と監視ツールの導入
